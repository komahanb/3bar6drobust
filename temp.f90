...
!!$
  ...
  dat(1000+7)=5000.0    ! psi tensile_sigma2_max=dat(7)
  dat(1000+8)=5000.0    ! psi tensile_sigma3_max=dat(8)
  !Compressive
  dat(1000+9)=5000.0    ! psi comp_sigma1_max=dat(9)
  dat(1000+10)=5000.0   ! psi comp_sigma2_max=dat(10)
  ...
  IDAT(2)=0 !1 = equals deterministic but with function values coming from the surrogate
...
!        write(*,'(x,a,i3,a,i3,a)')'>> Hello, from processor [',id_proc+1,' ] of [',num_proc,' ]'
 ...
 !       call Krigingestimate(N,N,x,sigmax,12,i,DAT(1001:1020),100,0,probtype,fmeantmp,fvartmp,fmeanprimetmp,fvarprimetmp)
  ...
  if(id_proc.eq.0) write(*,'(E13.4)'),g(i)
...
subroutine EV_JAC_G(TASK, N, X, NEW_X, M, NZ, ACON, AVAR, A,IDAT, DAT, IERR)
  ...
  integer ACON(NZ), AVAR(NZ), I, J, K, cnt
  double precision DAT(*),fmeanprimetmp(n),fvarprimetmp(n)
  real*8 :: fmeandbleprimetmp(n,n),fvardbleprimetmp(n,n)
  integer IDAT(*)
